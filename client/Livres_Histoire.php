...
if(!empty($_POST)){
  ...
  if(!empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivrehistoireName($_POST['searchTitre']);
  } 
  elseif(empty($_POST['searchTitre']) && !empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivrehistoireAuteur($_POST['searchAuteur']);
  }
  elseif(empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && !empty($_POST['searchPrix'])){
     
     if($_POST['searchPrix'] == 'prix bas'){
      $livres = searchLivrehistoirePrixBas();
     }else{
      $livres = searchLivrehistoirePrixEleve();
     }
...
<div class="row">
  <div class="col-lg-12">
    <form action="livres_Histoire.php" method="post">
    <ul id="portfolio-flters">
