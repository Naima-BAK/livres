...
$livres = getAllLivresCuisine();
$categories = getAllCategories();

if(!empty($_POST)){
           
  if(!empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivrecuisineName($_POST['searchTitre']);
  } 
  elseif(empty($_POST['searchTitre']) && !empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivrecuisineAuteur($_POST['searchAuteur']);
  }
  elseif(empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && !empty($_POST['searchPrix'])){
     
     if($_POST['searchPrix'] == 'prix bas'){
      $livres = searchLivrecuisinePrixBas();
     }else{
      $livres = searchLivrecuisinePrixEleve();
     }
...
<div class="row">
  <div class="col-lg-12">
    <form action="livres_cuisine.php" method="post">
    <ul id="portfolio-flters">
