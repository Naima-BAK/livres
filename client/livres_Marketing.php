...
$livres = getAllLivresMarketing();
$categories = getAllCategories();

if(!empty($_POST)){
           
  if(!empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivreMarketingName($_POST['searchTitre']);
  } 
  elseif(empty($_POST['searchTitre']) && !empty($_POST['searchAuteur']) && empty($_POST['searchPrix'])){
      $livres = searchLivreMarketingAuteur($_POST['searchAuteur']);
  }
  elseif(empty($_POST['searchTitre']) && empty($_POST['searchAuteur']) && !empty($_POST['searchPrix'])){
     
     if($_POST['searchPrix'] == 'prix bas'){
      $livres = searchLivreMarketingPrixBas();
     }else{
      $livres = searchLivreMarketingPrixEleve();
     }
...
<div class="row">
  <div class="col-lg-12">
    <form action="livres_Marketing.php" method="post">
    <ul id="portfolio-flters">
